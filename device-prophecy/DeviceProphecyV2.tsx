  ...
  return (
      <>
        <div className="container">
          <div className="column header failure-details">Failure Prophecy</div>
          <div className="column header devices">Device Names</div>
          <div className="column header">Recommended Actions</div>
        </div>
        {deviceProphecyItems.map((item, index) => (
            <div className="container" key={item.names[0]}>
              <div className="column failure-details">
                <div>{item.failureDetails}</div>
              </div>
              <div className="column devices">
                {/*{item.names.length > MAX_NUMBER_DEVICES &&*/}
                {/*collapsedStates[index] ? (*/}
                {/*  <div>*/}
                {/*<button onClick={() => toggleCollapse(index)}>Show All</button>*/}
                <div>
                  {item.names.slice(0, MAX_NUMBER_DEVICES).map((name) => (
                      <div key={name} style={{marginBottom: "8px"}}>
                        <label>
                          <input type="checkbox"/>
                          <a href="foo">{name}</a>
                  ...
                  >
                    <div>
                      ...and {Math.max(0, item.names.length - MAX_NUMBER_DEVICES)}{" "}
                      devices more
                    </div>
                    <AccordionItem id={item.raSuggested}>
                      {item.names
                      .slice(MAX_NUMBER_DEVICES, item.names.length)
                      .map((name) => (
                          <div key={name} style={{marginBottom: "8px"}}>
                            <input type="checkbox"/>
                            <a href="foo">{name}</a>
              ...
              <div className="column action">
                <ul>
                  <li className="li-without-border">
                    <ToastContainer/>
                    {item.raSuggested}
                    <br/>
                    <br/>
                    <button
                        onClick={() => handleRunActionClick(item.raSuggested)}
                        title={item.raSuggested}
                    >
                      <svg
                          width="14"
                          height="14"
